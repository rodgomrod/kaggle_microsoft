...
joblib.dump(xgb_model, 'saved_models/xgb_model_1.pkl')
del X_train
del y_train
gc.collect()

print('Best score en el entrenamiento:', xgb_model.best_score)

if save_feature_importances:
    importance_df = pd.DataFrame()
    importance_df["importance"] = xgb_model.feature_importances_(importance_type='gain')
    ...
    plt.tight_layout()
    plt.savefig('importances/xgb_model_1_importances.png')

    importance_df.sort_values(by="importance", ascending=False) \
        .to_csv('importances/feature_importances_xgb_model_º.csv', index=True)
